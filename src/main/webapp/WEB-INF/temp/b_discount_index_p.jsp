            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <a href="${contextRoot}/addPercentages">
                                    <button>新增趴數折扣活動</button>
                                </a><br>
                                <hr>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-sm" id="table_id">
                                        ...
                                        <tbody>
                                        <c:forEach var="percentages" items="${page.content}">
                                            <tr>
                                                <td><c:out value="${percentages.percentageName}"/> <c:choose>
                                                    <c:when test="${percentages.percentageOnOff == '1'}">
																	<span
                                                                            style="color: green; background-color: lightgreen;">活動中</span>
                                                    </c:when>
                                                    <c:when test="${percentages.percentageOnOff == '0'}">
                                                        <span style="color: white; background-color: gray;">活動已結束</span>
                                                    </c:when>
                                                </c:choose></td>
                                                <td><fmt:formatDate pattern="yyyy-MM-dd "
                                                                    value="${percentages.percentageTime}"/></td>
                                                <td><c:out value="${percentages.percentageMinimumCharge}"/></td>
                                                <td><c:out value="${percentages.percentage}"/></td>
                                                <td>
                                                    <div>
                                                        <a class="btn btn-sm btn-light"
                                                           href="${contextRoot}/updatePercentages?id=${percentages.percentageId}">編輯</a>
                                                        <a class="btn btn-sm btn-danger"
                                                           href="${contextRoot}/deletePercentages?id=${percentages.percentageId}"
