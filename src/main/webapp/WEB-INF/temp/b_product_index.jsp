                                    ...
                                    <table class="table table-striped table-sm" id="table_id">
                                        ...
                                        <c:forEach var="products" items="${products}" varStatus="s">
                                            <tr>
                                                ...
                                                <td><a href="${contextRoot}/onePR?id=${products.productId}"><img
                                                        width="100"
                                                        src='<c:url value="/showImage1/${products.productId}"/>'></a>
                                                </td>
                                                <td><a href="${contextRoot}/onePR?id=${products.productId}"><c:out
                                                        value="${products.name}"></c:out></a></td>
                                                <td style="color: red;">NT$ <c:out
                                                        value="${products.price}"></c:out></td>
                                                <td style="color: green;"><c:out
                                                        value="${products.category.type}"></c:out></td>
                                                <td><c:out value="${products.detail1}"></c:out></td>
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${products.productStatus.id == 0}">
                                                            <button id="change-product-status-btn-${s.count}"
                                                                    type="button"
                                                                    class="btn btn-outline-warning product-status-btn"
                                                                    data-product_id="${products.productId}"
                                                                    data-product_status_id="${products.productStatus.id}"
                                                                    data-product_status_0="${productStatusList.get(0).status}"
                                                                    data-product_status_1="${productStatusList.get(1).status}"
                                                                    data-product_name="${products.name}">
                                                                <span><b><c:out
                                                                        value="${products.productStatus.status}"/></b></span>
                                                            </button>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <button id="change-product-status-btn-${s.count}"
                                                                    type="button"
                                                                    class="btn btn-warning product-status-btn"
                                                                    data-product_id="${products.productId}"
                                                                    data-product_status_id="${products.productStatus.id}"
                                                                    data-product_status_0="${productStatusList.get(0).status}"
                                                                    data-product_status_1="${productStatusList.get(1).status}"
