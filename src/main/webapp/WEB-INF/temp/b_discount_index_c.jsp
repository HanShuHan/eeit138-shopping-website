                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-sm" id="table_id">
                                        ...
                                        <tbody>
                                        <c:forEach var="Coupons" items="${page.content}">
                                            <tr>
                                                <td><c:out value="${Coupons.couponName}"/> <c:choose>
                                                    <c:when test="${Coupons.couponOnOff == '1'}">
																	<span
                                                                            style="color: green; background-color: lightgreen;">活動中</span>
                                                    </c:when>
                                                    <c:when test="${Coupons.couponOnOff == '0'}">
                                                        <span style="color: white; background-color: gray;">活動已結束</span>
                                                    </c:when>
                                                </c:choose></td>
                                                <td><fmt:formatDate pattern="yyyy-MM-dd "
                                                                    value="${Coupons.couponTime}"/></td>
                                                <td><c:out value="${Coupons.couponMinimumCharge}"/></td>
                                                <td><c:out value="${Coupons.coupon}"/></td>
                                                <td>
                                                    <div>
                                                        <a class="btn btn-sm btn-light"
                                                           href="${contextRoot}/updateCoupons?id=${Coupons.couponId}">編輯</a>
                                                        <a class="btn btn-sm btn-danger"
                                                           href="${contextRoot}/deleteCoupons?id=${Coupons.couponId}"
