            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">訊息列表</h5><br>
                                <div class="table-responsive">
                                    <table class="table table-striped table-sm" id="table_id">
                                        ...
                                        <c:forEach var="replys" items="${replys}">
                                            <tr>
                                                <td><a href="${contextRoot}/b_reply_single?id=${replys.id}"><c:out
                                                        value="${replys.title}"></c:out></a></td>
                                                <td><fmt:formatDate pattern="yyyy/MM/dd HH:mm:ss"
                                                                    value="${replys.added}"></fmt:formatDate></td>
                                                <td><c:choose>
                                                    <c:when test="${replys.replymessage == null}">
                                                        尚未回覆
                                                    </c:when>
                                                    <c:otherwise>
                                                        已回復於：<fmt:formatDate pattern="yyyy/MM/dd HH:mm:ss"
                                                                                 value="${replys.replyadded}"></fmt:formatDate>
                                                    </c:otherwise>
                                                </c:choose></td>
                                                <td>
                                                    <div>
                                                        <a class="btn btn-sm btn-danger"
                                                           href="${contextRoot}/deleteCustomer?id=${replys.id}"
